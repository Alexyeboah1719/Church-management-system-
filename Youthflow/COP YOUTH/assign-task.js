...
function loadEventsForAssignment() {
    ...
    const upcomingEvents = events.filter(event => {
        ...
        return eventDate && new Date(eventDate) >= new Date().setHours(0, 0, 0, 0);
    });
    ...
    upcomingEvents.forEach(event => {
        const option = `<option value="${event.id}">${event.title} - ${formatDate(event.event_date || event.date)}</option>`;
...
// Setup storage event listeners for real-time updates
function setupStorageEventListeners() {
    // Listen for localStorage changes
    window.addEventListener('storage', function(e) {
        if (e.key === 'events') {
            console.log('Events updated, refreshing assign task dropdown...');
            loadEventsForAssignment();
        } else if (e.key === 'members') {
            console.log('Members updated, refreshing member dropdown...');
            loadMembersForAssignment();
        }
    });
